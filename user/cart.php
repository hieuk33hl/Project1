					...
					if (!empty($_POST['quantity']) && $error == false) {
						...
						while ($row = mysqli_fetch_array($result)) {
							...
							if ($_POST['quantity'][$row['ma_sp']] > $row['sl_sp']) {     //nếu số lượng sản phẩm post lên > số sp có trong database thì đưa về = sl_sp trong database
								...
								$updateString .= " when ma_sp = " . $row['ma_sp'] . " then sl_sp - " . $_POST['quantity'][$row['ma_sp']];       //nối chuôĩ

							}
						}
						if ($GLOBALS['changed_cart'] == false) {  // nếu giỏ hàng k có gì thay dổi thì k làm bthuon
							//update `san_pham` set sl_sp = CASE when ma_sp = 1 then sl_sp - 1 END where ma_sp in (1)
				...
				<div class="cart__info-customer">
					<table cellspacing="0" cellpadding="0">
						...
						<tr>
							<td colspan="2">
								<label>Ghi chú </label>
								<textarea name="" id="" cols="30" rows="10" name="note"></textarea>
							</td>
						</tr>
					</table>
					<button name="order">
